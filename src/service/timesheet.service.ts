...
class TimesheetService {
  ...
  resume = async (
    ...
    try {
      const timesheet: ITimesheet | null = await Timesheet.findOne({
        _id: timesheetId,
        isRunning: false,
      });
      if (!timesheet) {
        throw new ApiError(404, "Timesheet isn't not found or not pause");
      }
      ...
      const isToday =
        timesheet.startTime >= startOfDay && timesheet.startTime <= endOfDay;

      if (!isToday) {
        throw new Error(
          "This timesheet is not from today and cannot be resumed."
        );
      }
  ...
  pauseTime = async (
    ...
    try {
      ...
      timesheet = await Timesheet.findOneAndUpdate(
        { timesheetId, isRunning: true, employeeId },
        { $set: { isRunning: false } }
      );
      if (!timesheet) {
        throw new ApiError(400, "Time isn't started!");
